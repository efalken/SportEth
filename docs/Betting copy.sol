...
contract Betting {
  ...
  function settle(
    ...
    for (uint32 i = 0; i < 32; i++) {
      ...
      // if no one bets on the match these numbers will both be zero, and it need not
      ...
      if ((betDatav[0] + betDatav[1]) > 0) {
        ...
        if (winningTeam != 2) {
          ...
          // winning bet assigned number 2 not a tie
    ...
    // dividing by 1e6 allows the data to be in uint64
