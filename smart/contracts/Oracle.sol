...
contract Oracle {
  ...
  function initPost(
    ...
    uint _gameStart = block.timestamp -
      ((block.timestamp - 1687564800) % 604800) +
      604800;
    for (uint256 i = 0; i < 32; i++) {
      ...
      // require((_starts[i] >= _gameStart) && (_starts[i] - _gameStart) < 216000);
    }
    gameStart = uint32(_gameStart);
  ...
  function ethClaim(address msgsender) internal returns (uint256 _ethOut0) {
    ...
    uint256 ethTot = uint256(adminStruct[msgsender].tokens) *
