...
<<<<<<< HEAD
  ...
  function bet(
    ...
    require(_betAmt >= MIN_BET, "bets must be at least 1.0 avax");
    require(
...
>>>>>>> cb2d77f1b22b0d85e176e7c6f445b1f278662915
    ...
    function bet(
        ...
        require(
            userStruct[msg.sender].counter < 16,
            "redeem bets to reset acct"
        );
        require(
            _betAmt <= int64(uint64(userStruct[msg.sender].userBalance)),
            "over capital balance"
...
<<<<<<< HEAD
  ...
  function redeem() external {
    ...
    for (uint256 i = 0; i < numberBets; i++) {
      ...
      // creates epoch~matchnumber~pick id to see if a winner via mapping
      // created in settle()
      ...
      if (outcomeMap[epochMatch] != 0) {
        ...
        // 0.05 of winnings, only winners get payoff
...
<<<<<<< HEAD
    // resets counter as opposed to deleting lastTransaction array
    // works the same but uses less memory
...
=======
     ...
     */
    function transmitUpdate(uint16[32] calldata _updateOdds)
        external
        onlyAdmin
    {
        odds = _updateOdds;
...
>>>>>>> cb2d77f1b22b0d85e176e7c6f445b1f278662915
    ...
    function decodeNumber(uint256 _matchNumber)
        internal
        view
        returns (uint64[4] memory vec1)
    {
